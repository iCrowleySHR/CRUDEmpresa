...
class Cargo
{
    ...
    {
        return (new Connection('cargo'))->insert([
    ...
    {
        return (new Connection('cargo'))->select($where, $order, $limit, $fields)->fetchAll(PDO::FETCH_ASSOC);
    }

    public static function update(string $where, array $values): bool
    {
        return (new Connection('cargo'))->update($where, [
            'nomeCargo' => $values['nome']
        ]);
    }
    ...
    {
        return (new Connection('cargo'))->delete($where);
    }
}